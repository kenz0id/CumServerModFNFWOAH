...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			...
			case 'gaisano':
				var bg:BGSprite = new BGSprite('cum server/gaisano', -800, -200, 0.9, 0.9);
				add(bg);
				
				lyndon = new FlxSprite(-300, 500);
				lyndon.frames = Paths.getSparrowAtlas('cum server/dad');
				lyndon.animation.addByPrefix('idle', 'lyndon', 24, false);
				lyndon.scrollFactor.set(0.9, 0.9);
				lyndon.antialiasing = true;
				add(lyndon);
				
			
				
			case 'robinsons':
				var bg:BGSprite = new BGSprite('cum server/robinsons', -600, -500, 0.9, 0.9);
				add(bg);
				
		    case 'cum building':
				var bg:BGSprite = new BGSprite('cum server/cum building', -1400, -1450, 0.9, 0.9);
				add(bg);
				
			case 'eddyworld':
				var bg:BGSprite = new BGSprite('cum server/eddyworld', -600, -400, 0.9, 0.9);
				add(bg);
				
			case 'antartica':
				var bg:BGSprite = new BGSprite('cum server/antartica', -800, -500);
				add(bg);
				
			case 'void':
				var bg:BGSprite = new BGSprite('cum server/moaning bg', -600, -500);
				add(bg);
				
				erynn = new FlxSprite(1300, 400);
				erynn.frames = Paths.getSparrowAtlas('cum server/erynnscared');
				erynn.animation.addByPrefix('idle', 'erynn traumatized', 24, false);
				erynn.scrollFactor.set(0.9, 0.9);
				erynn.antialiasing = true;
				add(erynn);
	...
	override function beatHit()
	{
		...
		if (gf != null && curBeat % Math.round(gfSpeed * gf.danceEveryNumBeats) == 0 && gf.animation.curAnim != null && !gf.animation.curAnim.name.startsWith("sing") && !gf.stunned)
		{
		 ...
		 if (curBeat % gfSpeed == 0) {
			curBeat % (gfSpeed * 2) == 0 ? {
			 iconP1.scale.set(1.1, 0.1);
			 iconP2.scale.set(1.1, 2.0);
		 
			 FlxTween.angle(iconP1, -15, 0, Conductor.crochet / 1300 * gfSpeed, {ease: FlxEase.sineOut});
			 FlxTween.angle(iconP2, 15, 0, Conductor.crochet / 1300 * gfSpeed, {ease: FlxEase.sineOut});
			} : {
			 iconP1.scale.set(1.1, 2.0);
			 iconP2.scale.set(1.1, 0.1);
		 
			 FlxTween.angle(iconP2, -15, 0, Conductor.crochet / 1300 * gfSpeed, {ease: FlxEase.sineOut});
			 FlxTween.angle(iconP1, 15, 0, Conductor.crochet / 1300 * gfSpeed, {ease: FlxEase.sineOut});
			}
		 
			FlxTween.tween(iconP1, {'scale.x': 1, 'scale.y': 1}, Conductor.crochet / 1250 * gfSpeed, {ease: FlxEase.sineOut});
			FlxTween.tween(iconP2, {'scale.x': 1, 'scale.y': 1}, Conductor.crochet / 1250 * gfSpeed, {ease: FlxEase.sineOut});
		...
		switch (curStage)
		{
			...
			case 'gaisano':
				lyndon.animation.play('idle', true);
				
			case 'void':
				erynn.animation.play('idle', true);
